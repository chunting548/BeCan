...
{
  private static final String TAG = "LIBRARY BECAN";
          ...
          if (NavigineSDK.initialize(mContext, D.USER_HASH, D.SERVER_URL))
          {
            NavigineSDK.loadLocationInBackground(D.LOCATION_NAME, 30,
              new Location.LoadListener()
              {
                ...
                {
                  mStatusLabel.setText("下載地圖失敗 (error " + error + ")! " +
                                       "請稍後再試 錯誤代碼 : XE00");
                }
                @Override public void onUpdate(int progress)
                {
                  mStatusLabel.setText("下載地圖中 : " + progress + "%");
                }
              });
          }
          else
          {
            //Error initializing NavigineSDK! Please, contact technical support
            mStatusLabel.setText("SDK載入失敗 錯誤代碼 : XE01");
          }
        }
        break;
    }
